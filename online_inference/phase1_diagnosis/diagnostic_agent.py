...
Begin your analysis:"""
    ...
    def _extract_diagnostic_report(self, response: str) -> Dict[str, str]:
        ...
        if not report_match:
            logger.warning("未找到 <DIAGNOSTIC_REPORT> 标签，使用原始响应")
            # 降级：尝试提取 Thought 内容
            thought_match = re.search(r'Thought:\s*(.+?)(?=Action:|$)', response, re.DOTALL | re.IGNORECASE)
            if thought_match:
                raw_text = thought_match.group(1).strip()
            else:
                raw_text = response
        ...
        logger.debug(f"✓ 提取诊断报告: facts={len(facts)} 字符, keywords={len(keywords)} 字符")
